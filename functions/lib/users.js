...
function getStatus(job) {
    let legacyJob = Object.assign(Object.assign({}, job), (job.status || {}));
...
function lastMsgFrom(data) {
    if (data.chat) {
        const chat = Object.keys(data.chat).map(k => {
            return Object.assign({ n: k.split("-")[0], uid: k.split("-")[1] }, data.chat[k]);
        }).sort((a, b) => {
            return (+b.n - +a.n);
        })[0] || {};
        return chat.uid;
...
function addToPrintCue(job) {
    ...
    if (label) {
        let toPrint = Object.assign(Object.assign({}, job), { label });
    ...
    if (fromUid !== data.uid) {
        return fs.doc(`users/${data.uid}`).get().then(async (ref) => {
            ...
            if (data.forCustomer) {
                ...
                bike = Object.assign({ color: {} }, data.theBike);
            }
            else {
                let bikeRef = await fs.doc(`users/${data.uid}/bikes/${data.bike}`).get();
                bike = Object.assign({ color: {}, color2: {} }, bikeRef.data());
            }
...
async function claim(uid, jid, jobDoc, ref) {
    ...
    try {
        ...
        await fs.doc(`users/${uid}`).set(Object.assign({}, job.forCustomer), { merge: true });
    }
    catch (e) {
        // eslint-disable-next-line no-console
        console.log(e);
    ...
    const chat = Object.assign(Object.assign({}, job.chat), jobDoc.chat);
    const { services = [], DueDate = " ", Due = " " } = job;
    // eslint-disable-next-line no-console
    console.log(" 3 claiming " + jid);
    return await ref.set(Object.assign(Object.assign({}, jobDoc), { chat, services, DueDate, Due, cliamed: true, path }), { merge: true });
}
...
exports.emailUpdate = functions.region('europe-west2').firestore.document('/bikeshop/{shop}/emails/{id}').onWrite(async (change, context) => {
    ...
    const before = Object.assign(Object.assign({ body: "" }, change.before.data()), { id, shop });
    const after = Object.assign(Object.assign({ body: "" }, change.after.data()), { id, shop });
...
async function sendEmails(uid, jid, before, after) {
    if (getStatus(before) != getStatus(after)) {
        ...
        if (move.indexOf("Ready => Collected") !== -1) {
            fs.doc(after.path).set(Object.assign(Object.assign({}, after), { paid: price(after.services) }));
        }
        ...
        if (after.forCustomer) {
            let bikeData = Object.assign({ color: {} }, after.theBike);
            theBike = `${bikeData.make} ${bikeData.model}`;
            bike = Object.assign({ color: {} }, after.theBike);
            mail = after.forCustomer.email;
            customer = Object.assign({}, after.forCustomer);
        }
        else if (after.bike) {
            let bikeRef = await fs.doc(`users/${after.uid}/bikes/${after.bike}`).get();
            bike = Object.assign({ color: {} }, bikeRef.data());
            let userRef = await fs.doc(`users/${after.uid}`).get();
            customer = Object.assign({}, userRef.data());
            mail = after.email;
        }
        else {
            // eslint-disable-next-line no-console
        ...
        let linkData = Object.assign({}, after);
        ...
        const moveData = Object.assign(Object.assign({ count: 0 }, moveDataRef.data()), { egData, last: hashNow() });
        await moveRef.set(Object.assign(Object.assign({}, moveData), { count: ++moveData.count, last: hashNow() }));
...
exports.jobClaim = functions.region('europe-west2').firestore.document('users/{uid}/jobs/{jid}').onCreate((doc, context) => {
    ...
    const after = Object.assign(Object.assign({}, doc.data()), { id: jid, uid });
...
exports.jobAddVersion = functions.region('europe-west2').firestore.document('users/{uid}/jobs/{jid}').onWrite((change, context) => {
    const { uid, jid } = context.params;
    const before = Object.assign(Object.assign({}, change.before.data()), { id: jid, uid });
    const after = Object.assign(Object.assign({}, change.after.data()), { id: jid, uid });
